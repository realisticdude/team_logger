    ...
    return (
      <div className="p-4 md:p-6 lg:p-8">
        <div className="max-w-4xl mx-auto text-center py-12">
          ...
          <Link to="/">
            <Button className="w-full sm:w-auto bg-blue-600 hover:bg-blue-700 text-white">Back to Dashboard</Button>
          </Link>
        </div>
      </div>
    );
    ...
    <div className="p-4 md:p-6 lg:p-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 mb-4 md:mb-6">
        <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-4 md:p-6">
          ...
          <p className="text-2xl md:text-3xl font-semibold text-gray-900 dark:text-white">{user.todayTime > 0 ? formatTime(user.todayTime) : '0h 0m'}</p>
        </div>
            ...
            <div className="flex-1 h-2 bg-gray-100 dark:bg-gray-700 rounded-full overflow-hidden max-w-[120px]">
              <div
                className={`h-full rounded-full ${
                  user.productivity >= 80 ? 'bg-green-500 dark:bg-green-400' : user.productivity >= 60 ? 'bg-yellow-500 dark:bg-yellow-400' : 'bg-red-500 dark:bg-red-400'
                }`}
        ...
        {screenshots.length === 0 ? (
          <div className="text-center py-8 md:py-12">
            ...
            <p className="text-sm md:text-base text-gray-500 dark:text-gray-400">
              {user.status === 'active' ? 'Screenshots will appear here as they are captured.' : 'This user is currently offline.'}
            </p>
          </div>
        ) : (
