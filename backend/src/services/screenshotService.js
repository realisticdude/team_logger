...
export const saveMetadata = async (userId, imageUrl, timestamp) => {
  const { data, error } = await supabase.from('screenshots').insert({ user_id: userId, image_url: imageUrl, timestamp }).select('id,user_id,image_url,timestamp').single();
  if (error) throw error;
  return data;
};
...
export const listScreenshotsByUser = async (userId, days = 7) => {
  ...
  const { data, error } = await supabase.from('screenshots').select('id,user_id,image_url,timestamp').eq('user_id', userId).gte('timestamp', since).order('timestamp', { ascending: false });
  if (error) throw error;
  return data;
};
...
export const deleteOldScreenshots = async (days = 7) => {
  ...
  const { data, error } = await supabase.from('screenshots').select('id,user_id,image_url,timestamp,storage_path').lt('timestamp', cutoff);
  if (error) throw error;
  for (const s of data) {
    if (s.storage_path) {
